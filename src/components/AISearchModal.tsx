...
export const AISearchModal: React.FC<AISearchModalProps> = ({
  ...
  const [aiUnderstanding, setAiUnderstanding] = useState({
    mood: 'Sâu lắng & Giàu xúc cảm',
    ending: 'Tươi sáng / Đọng lại hy vọng',
    genre: 'Drama / Sci-Fi / Anime',
  ...
  const sampleQueries = [
    ...
    'Tôi chỉ có 30 phút để xem một phim ngắn hay',
    'Cho tôi một series bí ẩn như Dark hoặc Severance',
    'Phim nhẹ nhàng êm dịu xem trước khi ngủ',
    'Anime giàu cảm xúc về ký ức và thời gian',
    'Phim sci-fi vũ trụ kỳ vĩ như Interstellar'
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#062B45]/85 backdrop-blur-md flex justify-center p-4 text-[#062B45] animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden my-auto p-6 sm:p-8 space-y-6 border border-[#087EA4]/20 text-left">
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b border-gray-100">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#EAF8FC] text-[#087EA4] flex items-center justify-center">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-3">
          <div className="relative flex items-center">
            <input
              ...
              className="w-full pl-5 pr-28 py-3.5 bg-[#FAF8F5] rounded-2xl border border-gray-200 text-sm font-medium text-[#062B45] focus:outline-none focus:border-[#087EA4] focus:ring-2 focus:ring-[#087EA4]/20 shadow-inner"
              autoFocus
            />
            <button
              type="submit"
              disabled={loading}
              className="absolute right-2 px-5 py-2 rounded-xl bg-[#087EA4] hover:bg-[#062B45] text-white text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
            >
              {loading ? (
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            <span className="text-xs text-gray-400 self-center mr-1">Gợi ý:</span>
            {sampleQueries.map((s, idx) => (
              <button
                ...
                className="px-3 py-1 rounded-full bg-[#EAF8FC] hover:bg-[#19A7C7]/20 border border-[#19A7C7]/20 text-xs text-[#062B45] transition-colors cursor-pointer"
              >
                {s}
              </button>
            ))}
        ...
        {hasSearched && (
          <div className="bg-[#EAF8FC]/80 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-[#19A7C7]/30 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-[#19A7C7]/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-[#062B45]">
                  AI Hiểu Yêu Cầu Của Bạn:
                </h3>
              </div>
              <span className="text-[11px] text-[#087EA4] font-medium">
                (Có thể chỉnh sửa nhanh bên dưới)
              </span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
              <div className="bg-white p-2.5 rounded-xl border border-[#19A7C7]/20">
                <span className="text-[10px] font-bold text-gray-400 block uppercase">
                  Tâm trạng (Mood)
                </span>
                <input
                  type="text"
                  value={aiUnderstanding.mood}
                  onChange={(e) =>
                    setAiUnderstanding({ ...aiUnderstanding, mood: e.target.value })
                  }
                  className="w-full text-xs font-bold text-[#062B45] bg-transparent border-none p-0 focus:outline-none"
                />
              </div>
              ...
              <div className="bg-white p-2.5 rounded-xl border border-[#19A7C7]/20">
                ...
                <input
                  type="text"
                  value={aiUnderstanding.ending}
                  onChange={(e) =>
                    setAiUnderstanding({ ...aiUnderstanding, ending: e.target.value })
                  }
                  className="w-full text-xs font-bold text-[#087EA4] bg-transparent border-none p-0 focus:outline-none"
                />
              </div>
              ...
              <div className="bg-white p-2.5 rounded-xl border border-[#19A7C7]/20">
                ...
                <input
                  type="text"
                  value={aiUnderstanding.pace}
                  onChange={(e) =>
                    setAiUnderstanding({ ...aiUnderstanding, pace: e.target.value })
                  }
                  className="w-full text-xs font-bold text-[#062B45] bg-transparent border-none p-0 focus:outline-none"
                />
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[380px] overflow-y-auto pr-1">
              {matchedResults.map((item, idx) => (
                <div
                  key={item.id}
                  className="flex gap-3 p-3 rounded-2xl bg-white border border-gray-200 hover:border-[#087EA4] shadow-xs transition-all text-left group"
                >
                  <img
                    src={item.posterUrl}
                    alt={item.title}
                  ...
                  <div className="flex-1 flex flex-col justify-between overflow-hidden">
                    <div>
                      <div className="flex items-center justify-between text-[11px] text-gray-500 mb-0.5">
                        ...
                        <span className="flex items-center gap-1 font-bold text-[#062B45]">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-[#087EA4] text-white text-xs font-medium hover:bg-[#062B45] transition-colors cursor-pointer"
                      >
                        Chi tiết
                      </button>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-gray-100 text-gray-700 text-xs font-medium hover:bg-gray-200 transition-colors cursor-pointer"
                      >
                        Nơi xem
                      </button>
                    </div>
